...
def identify_evs_per_roi(prep_polys, evs_in_replicates, sizes):
    """
    ...
    for repl_id in range(n_replicates):
        ...
        for ev in evs_in_replicates[repl_id]:
            for roi_id in range(n_polys):
                ...
                #el
                if prep_polys[roi_id].contains(point):
...
replicates, target_iteration, base_path = 6, 2340000, 'D:\\iterations\\v33\\amp20estrous\\' #'/home/cmoralesmx/ev_iters/v32/amp/apop_2h01,sec_09,dt01'
...
analysis_setup['evs_in_roi_size_replicate']] = identify_evs_per_roi(
...
def pickle_data_to_compressed_file(data, name):
    p = os.path.join('resources','analysis','output',f'{name}','.pickle.bz2')
