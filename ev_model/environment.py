...
def process_edge(segments,
    ...
    section_id -
    internal - if the shape is contained by another we use the normal,
               otherwise, we use the binormal
    """
    segments_text = []
    distance_between_points = evmetry.two_points_distance(p1, p2)
    if debug_log:
        segments_text.append(
            f'processing edge p1:({p1[0]:-06.3f}, {p1[1]:-06.3f}),'
            f'p2({p2[0]:-06.3f}, {p2[1]:-06.3f}) '
            f'dBwPts = {distance_between_points:-06.3f}\n')
    ...
    if distance_between_points >= avg_cell_diameter:
        ...
        if debug_log:
            segments_text.append(f'fits {cells_to_fit} cells, '
                                 f'target diameter: {target_cell_diameter}\n')
        p1p2 = p2 - p1

        if cells_to_fit >= 2:
            # for more than two cells, we divide the segment into n subsegments
            for st in range(cells_to_fit - 1):
                # compute coordinates of the new endpoint for the subsegment
                dest_p = evmetry.project_point_on_direction(
                    source=source_p, direction=p1p2, dist=target_cell_diameter)
                # compute midpoint between the source and the new end point
                ...
                if debug_log:
                    segments_text.append(
                        f'  >>> segment {st} from p1:({source_p[0]}, '
                        f'{source_p[1]}) to p2:({dest_p[0]}, {dest_p[1]})\n')
                source_p = dest_p
            ...
            dest_p = evmetry.project_point_on_direction(
                source=source_p,
                direction=p1p2,
                dist=1. / distance_between_points * avg_cell_diameter)
            mp = evmetry.middle_point(source_p, dest_p)
            ...
            if debug_log:
                segments_text.append(
                    f'  >> segment A from p1:({source_p[0]}, {source_p[1]}) '
                    f'to p2:({dest_p[0]}, {dest_p[1]})\n')
            source_p = dest_p
            ...
            if debug_log:
                segments_text.append(
                    f'  >> segment B from p1:({source_p[0]}, {source_p[1]}) '
                    f'to p2:({dest_p[0]}, {dest_p[1]})\n')
    else:
        # only one cell fits in this section
        mp = evmetry.middle_point(p1, p2)
        seg = evmetry.Segment(p1[0], p1[1], p2[0], p2[1], mp[0], mp[1],
                              inside_another)
        segments.append(seg)
        # check_normal(seg, internal)
        if debug_log:
            segments_text.append(f'  > segment from p1:({p1[0]}, {p1[1]}) to '
                                 f'p2:({p2[0]}, {p2[1]})\n')
...
def identify_segments_in_polygons(polygons, avg_cell_diameter):
    ...
    for i in range(n):
        ...
        for idx in range(len(e_coords) - 1):
            p1 = e_coords[idx]
            p2 = e_coords[idx + 1]
            segments = process_edge(segments,
                                    p1,
                                    p2,
                                    avg_cell_diameter,
                                    i,
                                    inside_another=inside_another)
    return segments


def compute_source_points_locations_in_cell(p1,
                                            p2,
                                            largest_ev_diameter_um=0.4,
                                            debug_log=False):
    """
    Each cell can have many source points if its size is larger than the
    largest sized EV. This function computes these source points per cell.
    """
    direction = p2 - p1
    sps = []
    d = evmetry.two_points_distance(p1, p2)
    max_possible_source_points_in_cell = int(d /
                                             (1.1 * largest_ev_diameter_um))
    distance_between_source_points = d / max_possible_source_points_in_cell
    if debug_log:
        mpspc = ((max_possible_source_points_in_cell -
                  1) if max_possible_source_points_in_cell > 1 else
                 max_possible_source_points_in_cell)
        print(
            f'direction: {direction}, d: {d:6.3f}, '
            f'max_possible_source_points_in_cell: {mpspc}, '
            f'distance_between_source_points: {distance_between_source_points}'
        )
    if max_possible_source_points_in_cell > 1:
        for i in range(1, max_possible_source_points_in_cell):
            sps.append(
                evmetry.project_point_on_direction(
                    source=p1,
                    direction=direction,
                    dist=i * distance_between_source_points))
    else:
...
def identify_cell_range(x, y, grid, grid_size):
    ...
    min_x = int(current_cell[0] -
                1 if current_cell[0] > 1 else current_cell[0])
    min_y = int(current_cell[1] -
                1 if current_cell[1] > 1 else current_cell[1])
    # print(f'min_x:{min_x}, min_y:{min_y}')
    if current_cell in grid:
        return current_cell, [
            c for c in [(x, y) for x in range(min_x, min_x + 3)
...
def filter_source_points(grid, largest_ev_diameter_um=0.4, debug_log=False):
    ...
    for k in grid.keys():
        ...
        neighborhoods = [(x, y) for x in range(minx, minx + 3)
                         for y in range(miny, miny + 3)]
        for cellid in [c for c in neighborhoods if c in grid]:
            if len(grid[cellid]) > 2:
                if debug_log:
                    print(f'would check the sp in the {len(grid[cellid])} '
                          f'cells at {cellid}')
                for comb in itertools.combinations(grid[cellid], 2):
                    if debug_log:
                        print(f'{comb[0]._id} locations: '
                              f'{comb[0].source_points_locations}')
                        print(f'{comb[1]._id} locations: '
                              f'{comb[1].source_points_locations}')
                    max_loops = len(comb[0].source_points_locations) + len(
                        comb[1].source_points_locations)
                    for _ in range(max_loops):
                        r1 = [
                            i for i in range(
                                len(comb[0].source_points_locations))
                        ]
                        r2 = [
                            j for j in range(
                                len(comb[1].source_points_locations))
                        ]
                        if debug_log:
                            print(r1)
                            print(r2)
                        for pair in [(e1, e2) for e1 in r1 for e2 in r2]:
                            ...
                            if (np.dot(
                                    comb[0].unit_normal, comb[1].unit_normal
                            ) <= 0 and evmetry.two_points_distance(
                                    comb[0].source_points_locations[pair[0]],
                                    comb[1].source_points_locations[pair[1]]) <
                                    min_space):
                                if debug_log:
                                    print(f'Will drop one of these indices: '
                                          f'{pair[0]} or {pair[1]}')
                                dropped_count += 1
                                if random.random() < 0.5:
                                    del (comb[0].source_points_locations[
                                        pair[0]])
                                    if len(comb[0].source_points_locations
                                           ) == 0:
                                        emptied.append((cellid, comb[0]))
                                    if debug_log:
                                        print(f'Chosen: {pair[0]}')
                                else:
                                    del (comb[1].source_points_locations[
                                        pair[1]])
                                    if len(comb[1].source_points_locations
                                           ) == 0:
                ...
                # checked = True
    print(
        "Removed", dropped_count, " potentially conflictive source points."
        "As a result,", len(emptied),
        "secretory cells have no source points. ' \
                  'These will become ciliary cells.")
    return grid, emptied
    ...
    for grid_id, empty_secretory in emptied:
        for secretory_id in range(len(grid[grid_id])):
            if grid[grid_id][secretory_id]._id == empty_secretory._id:
                ...
                new_c = elements.CiliaryCell(
                    empty_secretory._id, empty_secretory.p1[0],
                    empty_secretory.p1[1], empty_secretory.p2[0],
                    empty_secretory.p2[1], empty_secretory.mp[0],
                    empty_secretory.mp[1], empty_secretory.direction_length,
                    empty_secretory.internal)
...
def produce_cells(segments,
                  probabilistic=False,
                  threshold_secretory=0.8,
                  largest_ev_radius_um=0.2,
                  debug_log=False,
                  grid_size=5):
    """
    This function uses the provided segments to identify/assign the type of
    cell it will become.

    Edge case scenarios:
    - Two mutually facing secretory cells too close (d <
    2xlargest_ev_radius_um) to each other will send EVs out of bounds.
        Mitigation:
            1 Meassure the distance between segments.
            2 keep track of what segment originated the new Secretory cell
            3 Before a new Secretory cell is introduced, check the distance to
            the other segments already originating a Secretory cell
    ...
    for segment in segments:
        ...
        if segment.distance_p1p2 < largest_ev_diameter_um:
            ...
            # 2xlargest_ev_diameter_um, only one of them can become a
            # Secretory cell
            current_cell, cell_range = identify_cell_range(
                segment.mp[0], segment.mp[1], grid, grid_size)
            ...
            if current_cell not in grid:
                ...
                for g_cell_id in cell_range:
                    for secretory_cell in grid[g_cell_id]:
                        d = evmetry.two_points_distance(
                            segment.location, secretory_cell.location)
                        dp = np.dot(segment.unit_normal,
                                    secretory_cell.unit_normal)
        ...
        if passes_secretory_checks:
            cell = elements.SecretoryCell(cell_id, segment.p1[0],
                                          segment.p1[1], segment.p2[0],
                                          segment.p2[1], segment.mp[0],
                                          segment.mp[1],
                                          segment.direction_length,
                                          segment.internal)
            cell.source_points_locations = list(
                compute_source_points_locations_in_cell(
                    cell.p1, cell.p2, largest_ev_diameter_um))

            if debug_log:
                print(
                    f'''Cell {cell._id} had {len(cell.source_points_locations)}
...
def at_safe_distance_to_near_secretory_cells(secretory, segment,
    ...
    _, cell_range = secretory.nearest_cells_for_coordinates(
        segment.mp[0], segment.mp[1])
...
def produce_cell_grids(segments,
                       probabilistic=False,
                       threshold_secretory=0.8,
                       largest_ev_radius_um=0.2,
                       debug_log=False,
                       grid_size=5):
    """
    Tests a segment against the prevously allocated secretory cells to
    determine if a new Secretory cell can be produced. Otherwise, the
    new cell will be a Ciliary cell.

    Edge case scenarios:
    - Two mutually facing secretory cells very close
      (d < 2xlargest_ev_radius_um) to each other will send EVs out of bounds.
      Mitigation:
            1 Meassure the distance between segments.
            2 keep track of what segment originated the new Secretory cell
            3 Before a new Secretory cell is introduced, check the distance to
    ...
    for segment in segments:
        ...
        # radius of the largest EV we must create a ciliary cell
        if segment.distance_p1p2 > largest_ev_diameter_um:
            if at_safe_distance_to_near_secretory_cells(
                    secretory, segment, shortest_secretory_separation):
                if probabilistic and random.random() > threshold_secretory:
                    passes_secretory_checks = True
...
def generate_source_points_from_cells(cells, debug_log=False):
    ...
    print(f'MAX NUMBER OF SOURCE POINTS PER CELL is {max_spts_per_cell}, '
          f'{len(cells_with_n_sps[max_spts_per_cell])} '
          f'cells have such many cells')
    if 0 in cells_with_n_sps:
        print('Cells with 0 source points:', len(cells_with_n_sps[0]),
              'indices:', ', '.join([str(n) for n in cells_with_n_sps[0]]))
    else:
        print('No cells with 0 source points')
    return source_points, max_spts_per_cell


def generate_evs_from_source_points(max_evs,
                                    secretory,
                                    new_evs_interval_seconds,
                                    debug_log=True):
    """
    Generates the N requested EVs.
    We create a list of ids for the secretory cells which gets shuffled.
    Then, we randomly choose a cell to produce a new EV by iterating from this
    list. The actual source point is selected randomly
    """
    ...
    for i in range(max_evs):
        # generate a new EV in order to allocate the values non-dependent on
        # the current/previous position
        ...
        # To prevent false positive collisions, we displace the current
        # starting_point by the length of the EV radius
        # and on the same direction of the unit normal of the base source point
        new_point = evmetry.project_point_on_direction(starting_point,
                                                       sc.unit_normal,
                                                       displacement_distance)
        if debug_log:
            print(f'    new ev_radius:{ev.radius_um}, displacement_distance:'
                  f'{displacement_distance}], new_point:[x:{new_point[0]}, '
                  f'y:{new_point[1]}]')

        # assign the location for the EV
        ev.x = float(new_point[0])
        ev.y = float(new_point[1])
        # The base direction for the EV matches the unit normal of
        # the source point
        ...
        if np.abs(ev.vx) >= 10 or np.abs(ev.vy) >= 10:
            print(f'warning: {i} dirx:{starting_point.dirx} '
                  f'diry:{starting_point.diry}')
...
def generate_evs_in_circle(n, circle_radius, random_direction=False):
    ...
    for i in range(n):
        ...
        ev = evmetry.project_point_on_direction(ev, direction[0], direction[1],
                                                ev.radius_um)
...
def export_cell_distributions(base_filename,
                              threshold_secretory,
                              cells=None,
                              secretory=None,
                              ciliary=None,
...
def export_cell_distribution_grids(base_filename,
                                   threshold_secretory,
                                   cells=None,
                                   secretory=None,
                                   ciliary=None):
...
def load_polygons(section):
    ...
    if section not in sections:
        raise ValueError('section ', section, 'does not exist in sections',
                         sections)
    else:
...
def setup_experiment(version,
                     ...
                     new_evs_interval_seconds=20,
                     new_evs_threshold=0.97,
                     n=0,
                     debug_log=False,
    ...
    start = time.time()
    s = time.time()
    base_filename = sections[working_section][len('resources/img_polygons/'
                                                  ):-len('.pickle')]
    print(f'Base filename: {base_filename}')
    with open(sections[working_section], 'rb') as pickled_source:
        polygons = pickle.load(pickled_source)

    segments = environment.identify_segments_in_polygons(
        polygons, avg_cell_diameter)
    print(f'Computed {len(segments)} segments in all polygons. Elapsed '
          f'{time.time()-s:.2f}')

    s = time.time()
    # generate the hypothetical cell distributions
    cells, secretory, ciliary, grid = environment.produce_cells(
        segments,
        True,
        threshold_secretory,
        grid_size=np.ceil(avg_cell_diameter))
    print(f'OLD METHOD: Generated {len(cells)} cells. The distribution is: '
          f'{len(secretory)} secretory and {len(ciliary)} ciliary. '
          f'Elapsed {time.time()-s:.2f}')

    s = time.time()
    cells_g, secretory_g, ciliary_g = environment.produce_cell_grids(
        segments,
        True,
        threshold_secretory,
        grid_size=np.ceil(avg_cell_diameter))
    print(
        f'Generated {cells_g.total_elements} cells. The distribution is: '
    ...
    print(f'Updated lists: {len(cells)} cells, {len(secretory)} secretory, '
          f'{len(ciliary)} ciliary')
    ...
    environment.export_cell_distributions(base_filename, threshold_secretory,
                                          cells, secretory, ciliary)
    environment.export_cell_distribution_grids(base_filename,
                                               threshold_secretory, cells_g,
                                               secretory_g, ciliary_g)

    s = time.time()
    # the max EVs to generate is capped at len(secretory)
    if n >= len(secretory) / 4:
        n = int(len(secretory) / 4)
        print(f"WARNING: max EV introduced in initial state caped at 1/4 "
              f"len(secretory), or: {n}")

    new_evs = environment.generate_evs_from_source_points(
        n, secretory, new_evs_interval_seconds, debug_log=debug_log)
    if n > 1:
    ...
    persistency.save_0xml_file(
        new_evs,
        n,
        dt,
        degrees_of_freedom,
        base_filename,
        target_spts_per_cell,
        version=version,
        boundaries=True,
        ev_ev_collisions=True,
        cells=cells,
        new_evs=True,
...
def visualize_distributions(cells,
                            secretory,
                            ciliary,
                            threshold_secretory,
                            save=False,
    ...
    ax1.add_collection(
        mc.LineCollection(col_secretories, color='red', linewidths=2))
    if len(ciliary) > 0:
        ax1.add_collection(
            mc.LineCollection(col_ciliaries, color='blue', linewidths=2))
    ...
    if normals:
        collection_secretories_normals = np.array(
            [[s.mp,
              evmetry.project_point_on_direction(s.mp, s.normal, 1)]
             for s in secretory])
        col_sec_norm = np.zeros(
            (collection_secretories_normals.shape[0], 2, 2))
        col_sec_norm[:, :, 0] = collection_secretories_normals[:, :, 0]
        col_sec_norm[:, :, 1] = collection_secretories_normals[:, :, 1]
        ax1.add_collection(
            mc.LineCollection(col_sec_norm, color='black', linewidths=1))
        if len(ciliary) > 0:
            collection_ciliaries_normals = np.array(
                [[c.mp,
                  evmetry.project_point_on_direction(c.mp, c.normal, 1)]
                 for c in ciliary])
            col_cil_norm = np.zeros(
                (collection_ciliaries_normals.shape[0], 2, 2))
            col_cil_norm[:, :, 0] = collection_ciliaries_normals[:, :, 0]
            col_cil_norm[:, :, 1] = collection_ciliaries_normals[:, :, 1]
            ax1.add_collection(
                mc.LineCollection(col_cil_norm, color='black', linewidths=1))
    if autoscale:
        ax1.autoscale()
    else:
        if len(ciliary) > 0:
            max_x = np.max([
                np.max(col_ciliaries[:, :, 0]),
                np.max(col_secretories[:, :, 0])
            ])
            max_y = np.max([
                np.max(col_ciliaries[:, :, 1]),
                np.max(col_secretories[:, :, 1])
            ])
        else:
...
def reconstruct_boundaries_from_agents(all_cells,
                                       secretory,
                                       ciliary,
                                       avg_cell_diameter=5):
    """
    ...
    To do so, we need to identify which agents have shared edges and collect
    them.
