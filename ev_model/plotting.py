...
def number_concentration_per_roi(d, section, version, iteration, rois_version, 
        rois_per_class, ordering, signs = None, means = None, max_y=0):
    target = 'Total EVs'
    dt = []
    aggregateds = {}
    overlays = []
...
def freq_bars(means1, sd1, means2, sd2, probs, ver1,
        iter1, ordering, rois_per_class, rois_ver1, ver2, iter2, rois_ver2, columns = 6):
    plots = []
    ...
    for cname, colour in ordering:
        ...
        if len(mns1):
            h1 = hv.Bars(mns1).options(color=colour, alpha=0.7, ylim=(0.1, (maxy + maxsd) * 1.01),
                title=f"Number concentration {cname}. Mean + SD. Pre-Ov", xlabel='EV radius in um', 
                ylabel='Frequency', bgcolor="#E8DDCB")
            e1 = hv.ErrorBars(err1)

            h2 = hv.Bars(mns2).options(color=colour, alpha=0.7, ylim=(0.1, (maxy + maxsd) * 1.01),
                title=f"Number concentration {cname}. Mean + SD. Post-Ov", xlabel='EV radius in um', 
                ylabel='Frequency', bgcolor="#E8DDCB")
            e2 = hv.ErrorBars(err2)

            if len(lbls1) > 0:
                plots.append((h1 * e1 * hv.Labels(lbls1) + h2 * e2 * hv.Labels(lbls2)))
            else:
...
def size_bars_per_roi(means1, sd1, means2, sd2, probs, bin_edges, ver1,
        iter1, ordering, rois_per_class, rois_ver1, ver2, iter2, rois_ver2, columns = 6):
    plots = []
    bin_edges[0] = 0.04
    bin_edges[-1] = 0.60001
    ...
    for cname, colour in ordering:
        for i in range(len(means1)):
            if i+1 in rois_per_class[cname]:
                ...
                h1 = hv.Histogram((bin_edges, m1)).options(color=colour, alpha=0.7, ylim=(0.1, (maxy + maxsd) * 1.01),
                    title=f"ROI #{i + 1} size distribution. Mean + SD. Pre-Ov", xlabel='EV radius in um', 
                    ylabel='Frequency', bgcolor="#E8DDCB")
                e1 = hv.ErrorBars((bin_edges, m1, s1))

                h2 = hv.Histogram((bin_edges, m2)).options(color=colour, alpha=0.7, ylim=(0.1, (maxy + maxsd) * 1.01),
                    title=f"ROI #{i + 1} size distribution. Mean + SD. Post-Ov", xlabel='EV radius in um', 
                        ylabel='Frequency', bgcolor="#C8DDCB")
                ...
                if len(labels1) > 0:
                    plots.append((h1 * e1 * hv.Labels(labels1) + h2 * e2 * hv.Labels(labels2)))
                else:
                    plots.append((h1 * e1 + h2 * e2))
